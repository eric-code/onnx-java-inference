...
这类标准转换场景，但对于图像预处理（resize → normalize → transpose）、文本处理（tokenize → padding）等更复杂的流水线，仍需编写自定义
jar。

计划设计一套**算子编排系统**作为通用处理器的升级替代：

- **算子注册**：内置常用算子（resize、normalize、transpose、tokenize、pad、argmax、softmax 等），每个算子有明确的输入输出类型
- **声明式编排**：在 model.yml 中声明前后处理流水线，用算子名 + 参数编排 DAG，无需编写 Java 代码
- **可扩展**：支持通过 SPI 注册自定义算子，与内置算子混合编排

#### 线性流水线（简单场景）

算子按声明顺序依次执行，前一个的输出是后一个的输入，适合图像预处理等单链路场景：
...
preprocess:
  ...
  - id: input_ids
    op: extract_field
    params: { field: input_ids }
    inputs: [tokenize]           # 依赖 tokenize 的输出

  - id: attention_mask
    op: extract_field
    params: { field: attention_mask }
    inputs: [tokenize]           # 依赖 tokenize 的输出，与 input_ids 并行

  - id: pad_ids
    op: pad_sequence
    params: { max_length: 128, pad_value: 0 }
    inputs: [input_ids]          # 依赖 input_ids

  - id: pad_mask
    op: pad_sequence
    params: { max_length: 128, pad_value: 0 }
    inputs: [attention_mask]     # 依赖 attention_mask，与 pad_ids 并行
```
...
```
                tokenize
               /        \
       input_ids    attention_mask    ← 并行分支
          |               |
       pad_ids        pad_mask        ← 并行分支
          \              /
        合并为模型输入 ──→ {input_ids, attention_mask}
```
